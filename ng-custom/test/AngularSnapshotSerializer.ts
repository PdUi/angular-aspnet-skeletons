export class AngularSnapshotSerializer {
  printAttributes = (val, attributes, print, indent, colors, opts) => {
    return attributes
      .sort()
      .map(attribute => {
        return (
  ...
  public print = (val, print, indent, opts, colors) => {
    let result = '';
    let componentAttrs = '';
    ...
    if (attributes.length) {
      componentAttrs += this.printAttributes(
        val,
        attributes,
        print,
        indent,
